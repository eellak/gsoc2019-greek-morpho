import re
import sys
from parse import get_forms,wword
...
def add_verb(form,lemma,person,number,tense,mood,aspect,verbform,voice):
	#TODO wword (wword already does findall so the next line is not needed)
	all_forms = get_forms(form)
	for i in all_forms:
		print(i +' '+ lemma + ' ' + str(s(person)) + ' '+s(number)+' '+s(tense)+' '+s(mood)+' '+s(aspect)+' '+s(verbform)+' '+s(voice))
		wword(i,lemma,'VERB',person=person,number=number,tense=tense,mood=mood,aspect=aspect,verbform=verbform,voice=voice)

def parse_verb(html,lemma):
	

	#print(re.sub(r'\\\n','\n',re.escape(t))) # this produces the above
	#print(a.group('metoxi'))
	v = 'Act'
	for a in re.finditer(energ,html,re.DOTALL|re.UNICODE):
		add_verb(a.group('ENEST_A_ENIKO'),lemma,1,'Sing','Pres','Ind','Imp','Fin',v)
		add_verb(a.group('ENEST_B_ENIKO'),lemma,2,'Sing','Pres','Ind','Imp','Fin',v)
		add_verb(a.group('ENEST_G_ENIKO'),lemma,3,'Sing','Pres','Ind','Imp','Fin',v)
		add_verb(a.group('ENEST_A_PL'),lemma,1,'Plur','Pres','Ind','Imp','Fin',v)
		add_verb(a.group('ENEST_B_PL'),lemma,2,'Plur','Pres','Ind','Imp','Fin',v)
		add_verb(a.group('ENEST_G_PL'),lemma,3,'Plur','Pres','Ind','Imp','Fin',v)
		
		add_verb(a.group('AOR_A_ENIKO'),lemma,1,'Sing','Past','Ind','Perf','Fin',v)
		add_verb(a.group('AOR_B_ENIKO'),lemma,2,'Sing','Past','Ind','Perf','Fin',v)
		add_verb(a.group('AOR_G_ENIKO'),lemma,3,'Sing','Past','Ind','Perf','Fin',v)
		add_verb(a.group('AOR_A_PL'),lemma,1,'Plur','Past','Ind','Perf','Fin',v)
		add_verb(a.group('AOR_B_PL'),lemma,2,'Plur','Past','Ind','Perf','Fin',v)
		add_verb(a.group('AOR_G_PL'),lemma,3,'Plur','Past','Ind','Perf','Fin',v)

		add_verb(a.group('PARATATIKOS_A_ENIKO'),lemma,1,'Sing','Past','Ind','Imp','Fin',v)
		add_verb(a.group('PARATATIKOS_B_ENIKO'),lemma,2,'Sing','Past','Ind','Imp','Fin',v)
		add_verb(a.group('PARATATIKOS_G_ENIKO'),lemma,3,'Sing','Past','Ind','Imp','Fin',v)
		add_verb(a.group('PARATATIKOS_A_PL'),lemma,1,'Plur','Past','Ind','Imp','Fin',v)
		add_verb(a.group('PARATATIKOS_B_PL'),lemma,2,'Plur','Past','Ind','Imp','Fin',v)
		add_verb(a.group('PARATATIKOS_G_PL'),lemma,3,'Plur','Past','Ind','Imp','Fin',v)

		add_verb(a.group('METOXI'),lemma,None,None,None,None,'Imp','Conv',v)
		add_verb(a.group('AOR_APAREMFATO'),lemma,None,None,None,None,'Perf','Inf',v)
		
		#γιατί οριστική για αυτό;??????
		#TODO χωρις χρόνο
		add_verb(a.group('AOR_YPOT_A_ENIKO'),lemma,1,'Sing',None,'Ind','Perf','Fin',v)
		add_verb(a.group('AOR_YPOT_B_ENIKO'),lemma,2,'Sing',None,'Ind','Perf','Fin',v)
		add_verb(a.group('AOR_YPOT_G_ENIKO'),lemma,3,'Sing',None,'Ind','Perf','Fin',v)
		add_verb(a.group('AOR_YPOT_A_PL'),lemma,1,'Plur',None,'Ind','Perf','Fin',v)
		add_verb(a.group('AOR_YPOT_B_PL'),lemma,2,'Plur',None,'Ind','Perf','Fin',v)
		add_verb(a.group('AOR_YPOT_G_PL'),lemma,3,'Plur',None,'Ind','Perf','Fin',v)
		v = 'Pass'
