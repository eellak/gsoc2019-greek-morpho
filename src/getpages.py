...
if args.production:
	...
	nouns = ['Βασιλόπουλος','εκτοκισμός','εισπνοή','βόλος','Βιβή','τελωνείο','δάκρυ','σύμμαχος','ανώμαλος','διάκεντρος','Ααρών','σκληρός δίσκος','Αρμαγεδδώνας','ρουλεμάν','Κολοκοτρώνης','γιατρός','σεληνοτοπογράφος','Ανδρέας','Μαρία','Αθήνα','Λάρισα','Ελλάδα','δέκα','άνθρακας','εδεσματολόγιο']
...
for title in nouns:
	...
	for c in categories:
		if c == 'Κατηγορία:Κύρια ονόματα (νέα ελληνικά)':
			part = 'PROPN'
	for c in categories:
		if c in ['Κατηγορία:Ανδρικά επώνυμα (νέα ελληνικά)','Κατηγορία:Γυναικεία επώνυμα (νέα ελληνικά)']:
			tag = id_add(tag, 'Ant')
			tag = id_add(tag, 'Surname') # Surname
		if c in ['Κατηγορία:Γυναικεία ονόματα (νέα ελληνικά)', 'Κατηγορία:Ανδρικά ονόματα (νέα ελληνικά)']:
			tag = id_add(tag, 'Ant')
			tag = id_add(tag, 'GivenName') # Given Name
		if c == 'Κατηγορία:Πολυλεκτικοί όροι (νεα ελληνικά)':
			tag = id_add(tag, 'PolyTerm')
		if c == 'Κατηγορία:Τοπωνύμια (νέα ελληνικά)':
			tag = id_add(tag, 'Top')
		if c == 'Κατηγορία:Πόλεις (νέα ελληνικά)':
			tag = id_add(tag, 'City')
		if c == 'Κατηγορία:Χώρες (νέα ελληνικά)':
			tag = id_add(tag, 'Country')
		if c == 'Κατηγορία:Πόλεις της Ελλάδας (νέα ελληνικά)':
			tag = id_add(tag, 'Top')
			tag = id_add(tag, 'City')
			tag = id_add(tag, 'Greece')
		if c == 'Κατηγορία:Πρωτεύουσες της Ευρωπαϊκής Ένωσης (νέα ελληνικά)':
			tag = id_add(tag, 'Top')
		...
		if c == 'Κατηγορία:Χημικά στοιχεία (ελληνικά)':
			tag = id_add(tag, 'ChemElement')
		if c == 'Κατηγορία:Πληροφορική (ελληνικά)':
